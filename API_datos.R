...
#Por Universidad subparcial_desc=="Universidad Tecnológica Nacional"
...
View(dataed %>% 
  filter(subparcial_desc=="Universidad Tecnológica Nacional") %>%
  ungroup() %>%
...
unc_ipc_14_15_16<-dataed %>% 
  mutate(fecha = as.Date(fecha)) %>%  # convert to date if not already
  filter(actividad_id %in% c(14,15,16), subparcial_desc=="Universidad Tecnológica Nacional") %>% 
  group_by(fecha) %>% 
  summarise(credito_devengado = sum(credito_devengado)) %>%
    left_join(ipc, by = "fecha") %>%
  mutate(credito_devengado_real = credito_devengado/cumulative)
...
unc_ipc_14<-dataed %>% 
  mutate(fecha = as.Date(fecha)) %>%  # convert to date if not already
  filter(actividad_id %in% c(14), subparcial_desc=="Universidad Tecnológica Nacional") %>% 
  group_by(fecha) %>% 
  summarise(credito_devengado = sum(credito_devengado)) %>%
    left_join(ipc, by = "fecha") %>%
  mutate(credito_devengado_real = credito_devengado/cumulative)
  ...
  unc_ipc_16<-dataed %>% 
  mutate(fecha = as.Date(fecha)) %>%  # convert to date if not already
  filter(actividad_id %in% c(16), subparcial_desc=="Universidad Tecnológica Nacional") %>% 
  group_by(fecha) %>% 
  summarise(credito_devengado = sum(credito_devengado)) %>%
    left_join(ipc, by = "fecha") %>%
  mutate(credito_devengado_real = credito_devengado/cumulative)
...
unc_ipc_all<-dataed %>% 
  mutate(fecha = as.Date(fecha)) %>%  # convert to date if not already
  filter(actividad_id %notin% c(12,13),subparcial_desc=="Universidad Tecnológica Nacional") %>% 
  group_by(fecha) %>% 
  summarise(credito_devengado = sum(credito_devengado)) %>%
    left_join(ipc, by = "fecha") %>%
  mutate(credito_devengado_real = credito_devengado/cumulative)
...
unc_ipc_all<-dataed %>% 
  mutate(fecha = as.Date(fecha)) %>%  # convert to date if not already
  filter(actividad_id %in% c(12,13),subparcial_desc=="Universidad Tecnológica Nacional") %>% 
  group_by(fecha) %>% 
  summarise(credito_devengado = sum(credito_devengado)) %>%
    left_join(ipc, by = "fecha") %>%
  mutate(credito_devengado_real = credito_devengado/cumulative)
...
data2024ed %>% filter(impacto_presupuestario_mes==3, subparcial_desc=="Universidad Tecnológica Nacional" ) %>% pull(impacto_presupuestario_fecha)
data2023ed %>% filter(impacto_presupuestario_mes==3, subparcial_desc=="Universidad Tecnológica Nacional" ) %>% pull(impacto_presupuestario_fecha)